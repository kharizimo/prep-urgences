<template>
        ...
        <div class="evaluation-filter-grid">
          <div>
            ...
            <select class="form-select" v-model="store.STATES.niveau">
                ...
                <option v-for="(t,i) in store.CONSTANTES.niveau" :key="k" :value="t">{{ t }}</option>
            </select>
          </div>
          <div>
            ...
            <select class="form-select" v-model="store.STATES.province">
              ...
              <option v-for="(t,i) in store.CONSTANTES.province" :key="k" :value="t">{{ t }}</option>
            </select>
          </div>
          <div>
    ...
    <Teleport to="#custom-btn"><button class="btn btn-success">Ajouter</button></Teleport>
</template>
...
const changeNiveau=(event)=>{
  if(store.STATES.niveau=='Provinces'){
    EntiteDisabled.value=true
  }
}
</script>
